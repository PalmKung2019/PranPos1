...
import { 
  ...
  Globe, User, MapPin, CheckCircle2, MessageCircle, Heart, Settings, Command, LogOut
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-body overflow-x-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              <Button 
                variant="ghost" 
                size="icon" 
                className="rounded-full hover:bg-red-50 h-12 w-12 group" 
                onClick={async () => { 
                  playClickSound(); 
                  try {
                    await auth.signOut();
                    toast({ title: language === 'en' ? "Logged out successfully" : "ออกจากระบบเรียบร้อยแล้ว" });
                  } catch(e) {
                    console.error(e);
                  }
                }}
      ...
      <Dialog open={isCustomizeOpen} onOpenChange={setIsCustomizeOpen}>
        <DialogContent aria-describedby={undefined} className="rounded-[40px] border-none p-0 overflow-hidden max-w-[90vw] sm:max-w-md bg-white shadow-2xl">
      ...
      <Dialog open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
        <DialogContent aria-describedby={undefined} className="rounded-[40px] border-none p-8 bg-white max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-2xl font-headline font-bold text-primary text-center">{t.paymentMethod}</DialogTitle>
          </DialogHeader>
      ...
      <div className="fixed bottom-8 left-8 right-8 z-40 flex justify-between items-center pointer-events-none">
        ...
        <Button 
          className="pointer-events-auto h-16 px-6 sm:px-8 rounded-full bg-primary text-white font-bold shadow-2xl flex items-center gap-3 transition-transform hover:scale-105 active:scale-95" 
          onClick={() => { playClickSound(); setIsCartOpen(true); }}
        >
          <div className="relative">
            <ShoppingCart className="w-6 h-6" />
            {cart.length > 0 && (
              <span className="absolute -top-2 -right-2 w-5 h-5 bg-accent text-white text-[10px] font-bold flex items-center justify-center rounded-full animate-bounce shadow-lg border-2 border-white">
                {cart.reduce((a, b) => a + b.quantity, 0)}
              </span>
            )}
          </div>
          <span className="font-headline text-lg uppercase tracking-wider hidden sm:inline">ตะกร้าสินค้า</span>
          {cart.length > 0 && (
            <div className="flex items-center gap-2 ml-2 pl-2 border-l border-white/20">
              <span className="font-bold text-lg sm:text-xl">฿{calculateSubtotal().toLocaleString()}</span>
            </div>
          )}
        </Button>
      </div>
